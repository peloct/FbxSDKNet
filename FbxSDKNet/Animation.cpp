...
AnimationLayer^ AnimationStack::GetAnimationLayer(int index)
{
	return gcnew AnimationLayer(animStack->GetMember<FbxAnimLayer>(index));
}
...
double AnimationCurve::GetKeyTimeSecond(int index)
{
	return curve->KeyGetTime(index).GetSecondDouble();
}
